...
def fetch_openalex_data_for_sdg(sdg_id: str, publication_year: int, *, per_page: int = 200, session: requests.Session | None = None) -> List[Dict[str, Any]]:
    """
    ...
    params = {
        "filter": f"sustainable_development_goals.id:{sdg_id},publication_year: {int(publication_year)}",
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                for fut in as_completed(futures):
                    ...
                    try:
                        ...
                        print(f"[sdg {sdg}] fetched {len(rows)} works.")
                    except Exception as e:
                        print(f"[sdg {sdg}] failed: {e}")
